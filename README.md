...
## 🆕 2026-04-18 ハイライト（Phase 1 Day 1）

- **3 台の Pico 2W が WiFi 経由で安定稼働**し、約 5.5 時間の連続データ取得に成功
- **PHNN 研究の基礎データを取得**: 温湿度 H(x) と通信遅延 R(x) の経験分布を蓄積
- 3 ノードの温度に **階層的な相関構造**（pico-2/3 が r=0.987、pico-1 だけ独立）を確認
- **R(x) と H(x) の独立性**を実証（RTT スパイク 10 件全てで温度変化 ≤ 0.1 °C）
- 14:00 に pico-1 のみ -1.2 °C の異常イベントを観測 → **人間活動検知の proof-of-concept**

詳細は **[docs/phase1-results-2026-04-18.md](docs/phase1-results-2026-04-18.md)** を参照。

## 🆕 2026-04-18 実験①：隣接ノードの温度予測

- pico-2 / pico-3 の温湿度から **pico-1 の温度を予測** する回帰実験
- 訓練 374 / 検証 187 サンプル（19:10 〜 20:10、5 秒整列）
...
- **M1 (線形回帰) が MLP を上回り**、M0 比で誤差を **90.4 % 削減**
- 現在の温度範囲では pico-1 ≈ (T2, T3, H2, H3) がほぼ完全な線形関係
- 限界: 検証期間の温度レンジが 0.1 °C しかなく「易しすぎた」可能性
  → 明日は `experiment01_temp_prediction_overnight.py` で一晩データ再評価

詳細は **[docs/phase1-experiment01-2026-04-18.md](docs/phase1-experiment01-2026-04-18.md)** を参照。
